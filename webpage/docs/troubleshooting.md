...
Glass Fence UI loads, but you don't see the screen, and it gives you a `connection timeout` or `disconnected` error?
...
Check that your ephemeral port range [`GF_WEBRTC_EPR`](/docs/v3/configuration/webrtc#epr) is correctly exposed as a `/udp` port range.

In the following example, the specified range `52000-52100` must also be exposed using Docker. You can't map it to a different range, e.g. `52000-52100:53000-53100/udp`. If you want to use a different range, you must change the range in [`GF_WEBRTC_EPR`](/docs/v3/configuration/webrtc#epr) too.
...
If your IP is not correct, you can specify your own IP resolver using [`GF_WEBRTC_IP_RETRIEVAL_URL`](/docs/v3/configuration/webrtc#ip_retrieval_url). It needs to return the IP address that will be used.
...
services:
  glass-fence:
    ...
    environment:
      ...
      GF_WEBRTC_ICELITE: 1
      # highlight-start
      GF_WEBRTC_IP_RETRIEVAL_URL: https://ifconfig.co/ip
      # highlight-end
```

Or you can specify your IP address manually using [`GF_WEBRTC_NAT1TO1`](/docs/v3/configuration/webrtc#nat1to1):
...
If you want to use glass-fence only locally, you must put your local IP address here, otherwise, the public address will be used.

### Glass Fence works externally, but not locally {#works-externally-but-not-locally}
...
- Make sure [`GF_WEBRTC_NAT1TO1`](/docs/v3/configuration/webrtc#nat1to1) is blank and the [`GF_WEBRTC_IP_RETRIEVAL_URL`](/docs/v3/configuration/webrtc#ip_retrieval_url) address is working correctly (if unset, the default value is chosen).
...
### Glass Fence works locally, but not externally {#works-locally-but-not-externally}

Make sure that you are exposing your ports correctly.

If you put a local IP as `GF_WEBRTC_NAT1TO1`, external clients try to connect to that IP. But it is unreachable for them because it is your local IP. You must use your public IP address with port forwarding.
...
[] host
[{172.17.0.0/24  172.17.0.1 map[]}] glass-fence1-net
[] none
[{172.18.0.0/24  172.18.0.1 map[]}] glass-fence2-net
```
...
In this case, the host subnet is the same as `glass-fence2-net`, meaning that the internet stops working as soon as a second Docker network is created.
...
volumes:
# For google-chrome
- /data:/home/glassfence/.config/google-chrome
```
...
- The profile is not owned by the correct user. Make sure that the profile is owned by the `glass-fence` user in the container. You can check this by running the following command:

```bash
# Check the owner of the profile
docker exec -it <container-id> ls -la /home/glassfence/.config/google-chrome
# To change the owner
docker exec -it <container-id> chown -R glass-fence:glass-fence /home/glassfence/.config/google-chrome
```
...
Check your [`GF_WEBRTC_NAT1TO1`](/docs/v3/configuration/webrtc#nat1to1) or ensure that [`GF_WEBRTC_IP_RETRIEVAL_URL`](/docs/v3/configuration/webrtc#ip_retrieval_url) returns the correct IP.
...
I: [pulseaudio] client.c: Freed 0 "glass-fence"
...
See [related issue](https://github.com/ARXNDEV/glass-fence/issues/276).
